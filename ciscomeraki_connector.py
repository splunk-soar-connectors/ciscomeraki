...
from ciscomeraki_consts import (
    CISCOMERAKI_CONNECTIVITY_ENDPOINT,
    CISCOMERAKI_ERR_CONNECTIVITY_TEST,
    CISCOMERAKI_SUCC_CONNECTIVITY_TEST,
)


class CiscomerakiConnector(BaseConnector):
    """
    Ciscomeraki connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = CISCOMERAKI_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(CISCOMERAKI_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(CISCOMERAKI_SUCC_CONNECTIVITY_TEST)
...
if __name__ == "__main__":
    ...
    connector = CiscomerakiConnector()
