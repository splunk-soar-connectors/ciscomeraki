#!/usr/bin/python
# File: ciscomeraki_list_adaptive_policy_groups.py
#
...
class ListAdaptivePolicyGroups(BaseAction):
    """Class to handle the list adaptive policy groups action."""
    ...
    def execute(self):
        ...
        ret_val, response = self._connector._utils._make_rest_call(
            consts.LIST_ADAPTIVE_POLICY_GROUPS.format(organization_id=organization_id),
            self._action_result,
            "get",
        )
        ...
        try:
            for group in response:
                self._action_result.add_data(group)
            summary = {"total_groups": len(response)}
